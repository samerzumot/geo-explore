"""Configuration for REE prospectivity prototype.

All hard-coded parameters live here for quick region portability.
"""
...
class Region:
    ...
    max_lon: float
    max_lat: float
    # UTM EPSG for local work (Mountain Pass ~ EPSG:32611)
    utm_epsg: int
...
class ModelParams:
    random_state: int
    grid_resolution_m: int  # 30 or 90
    rf_n_estimators: int
...
class Config:
    ...
    def default(base_dir: str | Path = Path(__file__).resolve().parents[1]) -> "Config":
        ...
        region = Region(
            ...
            # Approx 50x50 km around Mountain Pass Mine
            min_lon=-116.6,
            min_lat=35.3,
            max_lon=-115.9,
            max_lat=35.8,
